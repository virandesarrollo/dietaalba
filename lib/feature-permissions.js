export function deriveFeatureCapabilities(features) {
  ...
  const canTrackWeight = assigned.has('track_health') || assigned.has('track_weight');
  const canTrackBloodPressure = assigned.has('track_health') || assigned.has('track_blood_pressure');
  const canTrackHealth = canTrackWeight || canTrackBloodPressure;
