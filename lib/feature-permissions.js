export function deriveFeatureCapabilities(features) {
  ...
  const canTrackSteps = assigned.has('track_steps');
  const canAccessSettings = assigned.has('access_settings') || canTrackSteps;
  ...
  const canTrackWater = assigned.has('track_water');
  const canTrackSnacks = assigned.has('track_snacks');
  const canTrackNightBinges = assigned.has('track_night_binges');
  const canUsePatientFriends = assigned.has('patient_friends');
  const canUseFriendChallenges = assigned.has('friend_challenges');
  const canTrackCalories = assigned.has('track_calories');
