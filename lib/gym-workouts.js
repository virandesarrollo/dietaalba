...
export function buildWorkoutExerciseCards(exercises, sets) {
  return [...exercises]
    .sort((left, right) => {
      const leftPosition = Number.isInteger(left.position) ? left.position : Number.POSITIVE_INFINITY;
      const rightPosition = Number.isInteger(right.position) ? right.position : Number.POSITIVE_INFINITY;
      return leftPosition - rightPosition || left.created_at.localeCompare(right.created_at) || left.id.localeCompare(right.id);
    })
    .map((exercise) => ({
    id: exercise.id,
    ...
    sets: sets
      ...
      .map((set) => ({ id: set.id, weightKg: set.weight_kg, reps: set.reps, isCompleted: set.is_completed })),
    }));
}
