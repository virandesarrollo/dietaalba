...
export function normalizeMealType(value) {
  const normalized = folded(value);
  return MEAL_TYPES.find((mealType) => folded(mealType) === normalized) ?? MEAL_ALIASES.get(normalized) ?? null;
}
...
function normalizeMeal(meal, fallbackType) {
  ...
  return {
    ...rest,
    meal_type: mealType ?? meal.meal_type ?? fallbackType,
    options: [{ title, ingredients, recipe_url: normalizeMarkdownUrl(recipe_url) }],
  };
}
...
export function parseTabularText(text) {
  ...
  if (rows.length > 1) {
    ...
    for (const [, weekday] of dayColumns) {
      if (seenDays.has(weekday)) {
        for (const mealType of MEAL_TYPES) errors[`_table.options.${weekday}.${mealType}.ambiguous`] = 'Columna de día repetida';
      }
  ...
  for (const [key, state] of assignments) {
    ...
    plan[weekday][MEAL_TYPES.indexOf(mealType)].options = state.options.map(({ option }) => option);
  }
  ...
  for (const weekday of WEEKDAYS) {
    for (const mealType of MEAL_TYPES) {
      if (!assignments.has(`${weekday}:${mealType}`)) {
        errors[`_table.missing.${weekday}.${mealType}`] = 'Falta una asignación explícita para este día y comida';
      }
    }
  }
