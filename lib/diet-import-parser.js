...
function normalizeMealList(sourceMeals) {
  const entries = Array.isArray(sourceMeals)
    ? sourceMeals.map((meal) => [undefined, meal])
    : sourceMeals && typeof sourceMeals === 'object'
      ? Object.entries(sourceMeals)
      : null;
  if (!entries) return sourceMeals;

  const meals = entries.map(([fallbackType, meal]) => normalizeMeal(meal, fallbackType));
  const aliasesByType = new Map();
  for (const [index, meal] of meals.entries()) {
    const sourceType = entries[index][1]?.meal_type ?? entries[index][0];
    const canonicalType = folded(meal?.meal_type);
    const sourceName = typeof sourceType === 'string' ? sourceType.trim() : '';
    if (canonicalType && sourceName) {
      if (!aliasesByType.has(canonicalType)) aliasesByType.set(canonicalType, new Set());
      aliasesByType.get(canonicalType).add(folded(sourceName));
    }
  }
  return meals.map((meal, index) => {
    const sourceType = entries[index][1]?.meal_type ?? entries[index][0];
    const sourceName = typeof sourceType === 'string' ? sourceType.trim() : '';
    return aliasesByType.get(folded(meal?.meal_type))?.size > 1 && sourceName
      ? { ...meal, meal_type: sourceName }
      : meal;
  });
}
...
function normalizePlan(value) {
  ...
  for (const [dayName, sourceMeals] of Object.entries(value)) {
    ...
    const meals = normalizeMealList(sourceMeals);
    plan[weekday] = Array.isArray(meals) && meals.some((meal) => (
      Array.isArray(meal?.options) && meal.options.some((option) => folded(option?.title) === 'libre')
    )) ? meals.map((meal) => ({
      meal_type: meal.meal_type,
      options: [{ title: 'Libre', ingredients: '', recipe_url: '' }],
    })) : meals;
  }
  return { plan, errors };
}
...
export function parseTabularText(text) {
  ...
  for (const weekday of WEEKDAYS) {
    if (plan[weekday].length === 0) errors[`${weekday}._day.groups`] = 'Debe contener al menos un grupo de comida';
  }
