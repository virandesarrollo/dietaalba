...
export function buildExternalAiPrompt() {
  return `Analiza el PDF adjunto y conviértelo en JSON estricto. Debe contener exactamente 7 días (monday a sunday) y exactamente 6 comidas por día: ${MEAL_TYPES.join(', ')}. Cada comida tendrá únicamente meal_type, title, ingredients y recipe_url, todos como texto. Conserva todos los hipervínculos del PDF y asocia cada uno con la comida donde aparece. En recipe_url escribe una URL directa http/https, sin sintaxis Markdown; usa cadena vacía cuando no exista y nunca null. Responde solo JSON, sin Markdown. No inventes datos ni URLs. Protege la privacidad: no incluyas nombres ni otros datos personales innecesarios.`;
}
