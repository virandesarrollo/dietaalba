...
export function buildExternalAiPrompt(sourceText) {
  return `Convierte el contenido siguiente en JSON estricto. Debe contener exactamente 7 días (monday a sunday) y exactamente 6 comidas por día: ${MEAL_TYPES.join(', ')}. Cada comida tendrá únicamente meal_type, title, ingredients y recipe_url, todos como texto. Responde solo JSON, sin Markdown. No inventes datos ni URLs: usa cadena vacía cuando falten. Protege la privacidad: no incluyas datos personales innecesarios ni información ajena a la dieta.\n\nContenido:\n${String(sourceText ?? '')}`;
}
