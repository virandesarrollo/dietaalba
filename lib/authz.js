...
export function deriveAvailableViews(capabilities, featureCapabilities = {}) {
  ...
  if (capabilities.canManageGroupPlans || capabilities.canManageGroupUsers) {
    views.push('patientControl');
  }
  if (featureCapabilities.canAccessSettings) views.push('settings');
  if (featureCapabilities.canTrackHealth) views.push('health');
  if (featureCapabilities.canUsePatientFriends) views.push('friends');
  if (
...
const APP_VIEWS = ['patient', 'training', 'health', 'friends', 'settings'];
const ADMIN_VIEWS = ['users', 'admin', 'gymAdmin', 'patientControl'];
