    ...
    <nav aria-label="Navegación de administración" className="space-y-2">
      <div className="space-y-2">
        {views.map((view) => {
          ...
          return <button key={view} type="button" onClick={() => router.push(item.path)} aria-current={selected ? 'page' : undefined} className={`min-h-12 flex w-full items-center gap-2 rounded-2xl px-3 py-2 text-left text-xs font-semibold transition ${selected ? 'bg-slate-800 text-white' : 'bg-white/70 text-slate-600 hover:bg-white hover:text-rose-500'}`}>
            <Icon aria-hidden="true" size={16} />{item.label}
          </button>;
        })}
      </div>
      <button type="button" onClick={() => router.push('/')} className="min-h-12 flex w-full items-center gap-2 rounded-2xl border border-slate-200 bg-white/70 px-3 py-2 text-left text-xs font-semibold text-slate-600 transition hover:bg-white hover:text-rose-500">
