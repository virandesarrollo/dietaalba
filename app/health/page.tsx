...
export default function Health() {
  ...
  function toggleEdit(row: R) { if (editingId === row.id) { setF(null); setEditingId(null); return; } edit(row); }
  ...
  return <main className="theme-page mx-auto min-h-screen max-w-md p-5 pb-28"><header className="flex items-center justify-between"><button aria-label="Día anterior" onClick={() => setD(day(d, -1))} className="min-h-12 min-w-12"><ChevronLeft /></button><b>Salud</b><div className="flex items-center gap-2"><button aria-label="Día siguiente" onClick={() => setD(day(d, 1))} className="min-h-12 min-w-12"><ChevronRight /></button><AccountMenu /></div></header><label className="relative my-4 block text-center">{formatWorkoutDate(d)}<input aria-label="Seleccionar fecha de salud" type="date" value={d} onChange={(e) => setD(e.target.value)} className="absolute inset-0 opacity-0"/></label>{error && <p role="alert" className="mb-3 rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}{rows.map((r, index) => <div key={r.id}>{rows.length > 1 && <button type="button" onClick={() => toggleEdit(r)} className="mb-3 w-full rounded-3xl bg-white p-4 text-left shadow-sm"><b>{measurementOrdinals[index] ? `${measurementOrdinals[index]} medición: ` : "Medición de las "}{new Date(r.recorded_at).toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" })}</b></button>}{editingId === r.id && f && form}</div>)}{f && editingId === null ? form : <button onClick={() => { setEditingId(null); setF(blankNow(d)); }} className="min-h-14 w-full rounded-2xl bg-slate-800 text-white"><Plus className="inline" /> Añadir medición</button>}<AppMobileNavigation current="health" /></main>;
}
