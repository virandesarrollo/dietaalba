    ...
    <main className="theme-page min-h-screen text-slate-700 pb-28 max-w-md mx-auto relative font-sans">
      ...
      {currentTab === 'plan' && (
        <section className="px-5 mt-6">
          ...
          {canTrackSnacks && snacks.map((snack) => <button key={snack.id} type="button" onClick={() => { if (!isHistoricalDay) { setEditingSnack(snack); setSnackText(snack.text); setSnackTime(new Intl.DateTimeFormat('en-GB', { timeZone: 'Europe/Madrid', hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }).format(new Date(snack.recorded_at))); } }} className="mb-3 block w-full rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-left text-xs text-red-800">⚠ Picoteo {new Date(snack.recorded_at).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}: {snack.text}{!isHistoricalDay && <span className="ml-2 font-bold">Editar</span>}</button>)}
