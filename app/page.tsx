...
export default function Home() {
  ...
  const { canRateRecipes, canSendReport, canOpenNotes, canAccessSettings, canTrackWater, canTrackSnacks, canTrackNightBinges, canUsePatientFriends } = featureCapabilities;
  ...
  async function saveSnack() {
    const { error } = await supabase.rpc('save_my_snack_log', { p_text: snackText });
  ...
  async function updateSnack() { if (!editingSnack) return; const { error } = await supabase.rpc('update_my_snack_log', { p_id: editingSnack.id, p_text: snackText }); if (error) setPlanError('No se pudo editar el picoteo.'); else { setEditingSnack(null); setSnackText(''); void fetchData(selectedDate); } }
          ...
          {loading ? (
            ...
            <div className="space-y-3.5">
              {Object.entries(groupedMeals).map(([mealType, options]) => {
                ...
                {canTrackSnacks && <section className="py-1 text-center"><button type="button" onClick={() => { setSnackDialogMealType(mealType); setShowSnackDialog(true); }} disabled={isHistoricalDay} className="min-h-11 rounded-2xl bg-red-600 px-4 text-xs font-bold text-white shadow-md disabled:opacity-40">⚠ Voy a picar</button></section>}
