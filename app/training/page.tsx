...
export default function TrainingPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const [catalogResult, dailyResult, setsResult, stepResult] =
          await Promise.all([
            supabase
              .from("gym_exercises")
              .select(
  ...
  async function copySelectedWorkout() {
    ...
    try {
      const result = await supabase.rpc("copy_my_gym_workout", {
        p_source_date: selectedCopySource.date,
        p_target_date: workoutDate,
      });
      if (result.error) {
  ...
  async function moveExercise(exercise: DailyExercise, direction: -1 | 1) {
    ...
    try {
      setFeedback("");
      const result = await supabase.rpc("move_my_gym_workout_exercise", {
        ...
        >
          <header className="flex min-h-12 items-center justify-between gap-2 border-b pb-3">
            ...
            <div className="flex shrink-0 gap-1">
              ...
              <button
                type="button"
                disabled={saving || cardIndex === cards.length - 1}
